...
### 객체지향보다 효율적인 경우
- 대용량 데이터 처리시
    - 데이터를 객체 생성 후 처리하는 것 보다 데이터를 바로 처리하는 것이 속도에 유리
    - 멀티코어 CPU 에서 데이터를 병렬 처리하고 취합할 때 객체보다는 함수가 유리
- 이벤트 지향 프로그래밍 (이벤트가 실행하면 핸들러 메소드 함수 실행)에 적합
    - 반복적인 이벤트 처리를 핸들러 객체보다는 핸들러 함수가 적합

### 람다식
- 람다 계산법에서 사용된 식을 프로그래밍 언어에 접목
- 익명 함수를 생성하기 위한 식
- 코드가 간결
- 컬렉션 요소를 필터링 또는 매핑하여 쉽게 집계할 수 있다.
...
- 장점
    - 검색속도가 빨라질 수 있다.
    - 해당 쿼리의 부하가 줄어 결국 시스템 전체 성능이 향상된다.
- 단점
    - 인덱스가 DB 공간을 차지해서 추가 공간이 필요해 지는데 약 10% 정도 필요하다
    - 인덱스 생성 시 시간이 소요된다.
    - 데이터 변경(insert, update, delete) 이 자주 일어날 경우 오히려 성능이 나빠질 수 있다.

- clustered index
    - 군집화된 인덱스
    - 인덱스와 데이터가 군집
    - 인덱스 자체의 리프 페이지가 곧 데이터이다. 따라서 인덱스 자체에 데이터가 포함되어 있다 할 수 있다.
    - PK 지정시 자동으로 클러스터 인덱스 생성
    - PK 가 없고 UK가 not null 인 경우 자동 클러스터 인덱스 생성 (PK 가 있으면 PK가 클러스터 인덱스가 된다.)
    - 순서대로 테이블 당 하나씩만 존재 
    - 행 데이터를 인덱스로 지정한 열에 맞춰 자동 정렬한다.
    - 범위 검색 시 좋음(between, >, <)
    - order by 절에 자주 나오는 열도 클러스터 인덱스가 유리하다.
        - 이미 정렬되있기 때문
    - insert, update, delete 시 페이지 분할, 정렬이 일어나기 때문에 성능이 안 좋다.
    - 인덱스 페이지에 데이터 페이지가 같이 있다.
    - 검색에서는 클러스터 인덱스가 비클러스터 인덱스보다 빠르다.
- non-clustered index
    - 데이터와 연결되어있지 않고 인덱스와 연결되어있다. (간접참조)
    - 순서와 상관없고 데이터를 가지지 않는다.
    - Unique (nullable) 지정 시 자동으로 비클러스터 인덱스 생성
    - 한 테이블 당 여러개 가질 수 있음
    - 추가 저장 공간 필요 (약 10% 정도)
    - insert 시 추가 작업 필요 (인덱스 생성)
    - 카디널러티가 높은걸 골라야 함
    - 범위 검색 시 성능이 안좋음
    - insert, update, delete 시 페이지 분할이 없어 클러스터 인덱스에 비해 성능이 좋다.
    - 데이터 페이지는 그대로 이고 별도의 페이지에 인덱스가 구성된다.
    - 비클러스터의 인덱스의 리프 페이지는 데이터가 아니라 데이터가 위치하는 주소 값이다.
    - 클러스터, 비클러스터 인덱스 혼용하는 경우 인덱스 삭제 시 비클러스 인덱스를 먼저 삭제하는게 좋다.

### 인덱스 내부 작동
- B-Tree(Balanced Tree, 균형 트리)
    - root node (root 페이지) 와 leaf node (leaf 페이지)
    - mysql 에서는 노드를 페이지라 부른다.
    - 페이지 당 최소 mysql 은 16kbyte 저장 (oracle 은 8kb)
    - insert 작업시 페이지가 꽉 찼다면 페이지 분할이 발생한다. 이 때 성능이 느려진다.
    

![인덱스 구조](../img/B-Tree.png)
...
- where 절에 사용되더라도 자주 사용해야 가치가 있다.
    - 가끔 쓸거면 안 만드는게 낫다.
- 데이터 중복도가 높은 열은 효과가 없다.
    - 카디널러티가 높을 수록 좋다.
- join 에 자주사용 되는 열에 인덱스를 만드는게 좋다.
- insert/update/delete 가 얼마나 자주 일어나는지 고려해야 한다.
    - 인덱스는 읽기 에서만 성능을 향상시키며, 데이터 변경에는 오히려 부담을 준다.
    - 데이터 변경이 자주 일어나는 경우 오히려 클러스터 인덱스가 없는게 나을 수도 있다.

### 인덱스를 제거해야 하는 경우
- 데이터 변경(insert, delete, update) 가 자주 일어나는 경우 클러스터 인덱스가 없는게 낫다.
- where 절에서 사용되지 않는 인덱스는 제거하는게 낫다.
...
### 트랜잭션 특성
1. 원자성 (Atomicity)
    - 트랜잭션은 연산의 집합이 하나의 단위로 작용한다.
    - 모두 반영(commit)되거나 하나라도 잘못되면 모두 이전 상태로 되돌려야(rollback) 한다.
2. 일관성 (Consistency)
    - 고정 요소는 실행전과 후가 같아야 한다.
    - 트랜잭션이 실행된 후에도 데이터베이스 상태는 무결성이 유지되고 모순되지 말아야 한다.
3. 독립성 (Isolation)
    - 트랜잭션 간에는 개입이 있어선 안된다.
    - 같은 자원에 대해 여러 개의 트랜잭션이 동시에 사용할 수 없다>
4. 지속성 (Durability)
    - 성공적으로 완료된 트랜잭션 결과는 DB에 영구 반영되어야 한다.  
...
- 컴파일 시 강한 타입 체크를 할 수 있다.
    - 컴파일 시에 미리 타입을 강하게 체크하여 에러를 사전에 방지한다.
- 타입 변환 과정이 필요 없다.

### 제네릭 타입
> 타입을 파라미터로 가지는 클래스와 인터페이스
    - 선언 시 클래스 또는 인터페이스 뒤에 <> 부호가 붙음
    - 괄호 사이에는 타입 파라미터가 위치한다.
